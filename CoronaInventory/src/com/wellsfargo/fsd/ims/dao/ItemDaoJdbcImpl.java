...
public class ItemDaoJdbcImpl implements ItemDao {
	...
	public static final String INS_ITEM_QRY = "INSERT INTO items(icode,title,unit,costPrice) values(?,?,?,?)";
	public static final String UPD_ITEM_QRY = "UPDATE items SET title=?,unit=?,costPrice=? WHERE icode=?";
	public static final String DEL_ITEM_QRY = "DELETE FROM items WHERE icode=?";
	public static final String SEL_ITEM_QRY_BY_ID = "SELECT icode,title,unit,costPrice FROM items WHERE icode=?";
	public static final String SEL_ALL_ITEMS_QRY = "SELECT icode,title,unit,costPrice FROM items";
	...
	public Item add(Item item) throws ImsException {
		if (item != null) {
			try (Connection con = ConnectionFactory.getConnection();
				...
				pst.setDouble(4, item.getCostPrice());
	...
	public Item save(Item item) throws ImsException {
		if (item != null) {
			try (Connection con = ConnectionFactory.getConnection();
				...
				pst.setDouble(3, item.getCostPrice());
	...
	public Item getById(Integer icode) throws ImsException {
		...
		try (Connection con = ConnectionFactory.getConnection();
			...
			if (rs.next()) {
				...
				item.setCostPrice(rs.getDouble(4));
